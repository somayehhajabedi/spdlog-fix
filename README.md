...
1. level.h  : Data race
   ...
   It is not correct because this is a data race leads to undefined behavior, but It is intermittent because the behavior depends on thread scheduling 
...
2. shutdown.h  : use after free ( dangling pointer )
   ...
   It is intermittent because the failure depends on timing between the logging thread and shutdown. Some runs may be fine, while others may cause use after free behavior.  
...
4. sink_callback.h :   recursive / stack overflow / crash

   The problem is the custom sink calls spdlog::info() function from inside sink_it_() function.
   That can lead to recursion, crashs based on timing.
...
6. string_uaf.h  Use after free  ( dangling pointer )
